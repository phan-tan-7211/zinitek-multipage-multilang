...
The shape wrapper intentionally has no CSS box shadow: `shadow-lg` casts a rectangular shadow around the transparent hexagon SVG and creates a visible square on light backgrounds. Keep the separate Sanity-controlled glow so the mark blends into the surrounding surface without a square tile.
