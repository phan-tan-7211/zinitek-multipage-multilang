...
// Import Dynamic Icon để hiển thị biểu tượng từ Sanity
...
interface Service {
  ...
  // Trường dữ liệu mới để phục vụ chuyển đổi ngôn ngữ
...
export function ServicePageContent({ service, relatedServices, lang, dict }: ServicePageContentProps) {
  const thamChieuGiaoDien = useRef(null)
  const dangTrongTamNhin = useInView(thamChieuGiaoDien, { once: true, margin: "-100px" })

  /**
   * LOGIC QUAN TRỌNG: ĐĂNG KÝ BẢN DỊCH VÀO HỆ THỐNG TOÀN CỤC
   * Mục đích: Giúp thanh điều hướng (Navigation) biết được Slug tương ứng của các ngôn ngữ khác.
   */
  useEffect(() => {
    if (service.banDichTuongUng && service.banDichTuongUng.length > 0) {
      ...
      // Gán vào đối tượng window để Navigation.tsx có thể đọc được
      (window as any).zinitekTranslations = banDoDichThuat;
    }

    // Dọn dẹp dữ liệu khi người dùng rời khỏi trang
    return () => {
  ...
  return (
    <>
      ...
      <section className="relative pt-40 pb-20 overflow-hidden">
        ...
        <div className="container mx-auto px-4 lg:px-6 relative z-10">
          ...
          >
            ...
            <ChevronRight className="w-4 h-4 text-muted-foreground" />
            <span className="text-[#f97316] font-medium">{service.shortTitle || service.title}</span>
          </motion.nav>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              >
                <DynamicIcon iconData={service.icon} className="w-4 h-4" />
                {dict?.services?.badge || "Dịch vụ"}
              </motion.div>
      ...
      <section ref={thamChieuGiaoDien} className="relative py-24 lg:py-32">
              ...
              <div className="grid grid-cols-2 gap-4">
                {service.specs.map((thongSo, chiSo) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-muted-foreground">{thongSo.label}</div>
                  </motion.div>
                ))}
              </div>
            </motion.div>
        ...
        <div className="container mx-auto px-4 lg:px-6 relative z-10">
          ...
          >
            <h2 className="font-serif text-3xl md:text-4xl font-bold text-foreground mb-4">
              {service.labels?.processTitle || "Quy trình"} <span className="italic text-[#f97316]">làm việc</span>
            </h2>
          ...
          <div className="relative">
            ...
            <div className="space-y-8 lg:space-y-0">
              {service.process.map((buoc, chiSo) => (
                <motion.div
                  key={`step-${chiSo}`}
                ...
                >
                  <div className={`lg:w-1/2 ${chiSo % 2 === 0 ? "lg:pr-16 lg:text-right" : "lg:pl-16"}`}>
                    <div className="p-6 rounded-xl bg-[#0f172a] border border-[#334155]/50 hover:border-[#f97316]/30 transition-colors">
                      ...
                      <p className="text-muted-foreground">{buoc.description}</p>
                    </div>
                  </div>
      ...
      <section className="relative py-24 lg:py-32">
        <div className="container mx-auto px-4 lg:px-6">
          ...
          >
            <h2 className="font-serif text-3xl md:text-4xl font-bold text-foreground mb-4">
              {service.labels?.relatedTitle || "Dịch vụ"} <span className="italic text-[#f97316]">liên quan</span>
            </h2>
            <p className="text-muted-foreground">
              Khám phá các giải pháp kỹ thuật khác tại ZINITEK
            </p>
          </motion.div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {relatedServices.map((dichVuLienQuan, chiSo) => {
              return (
                <motion.div
                  key={`related-${dichVuLienQuan.slug}-${chiSo}`}
                  initial={{ opacity: 0, y: 50 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.6, delay: chiSo * 0.15 }}
                >
                  ...
                  >
                    ...
                    <h3 className="font-serif text-lg font-bold text-foreground mb-2 group-hover:text-[#f97316] transition-colors">
                      {dichVuLienQuan.title}
                    </h3>
                    <p className="text-sm text-muted-foreground line-clamp-2 mb-4">
                      {dichVuLienQuan.description}
                    </p>
            ...
            <div className="relative flex flex-col md:flex-row items-center justify-between gap-8">
              <div>
                <h3 className="font-serif text-2xl md:text-3xl font-bold text-foreground mb-2 text-balance">
                  Bạn cần tư vấn về {service.shortTitle || service.title}?
                </h3>
                <p className="text-muted-foreground">
              ...
              <Button asChild size="lg" className="flex-shrink-0 bg-gradient-to-r from-[#f97316] to-[#ea580c] hover:from-[#ea580c] hover:to-[#c2410c] text-[#020617] font-semibold shadow-lg shadow-[#f97316]/25 hover:shadow-[#f97316]/40 transition-all duration-300 hover:scale-105">
                <Link href={`/${lang}/contact`}>
                  {dict?.services?.request_consult || "Yêu cầu tư vấn"}
                  <ArrowRight className="ml-2 w-5 h-5" />
                </Link>
              </Button>
            </div>
