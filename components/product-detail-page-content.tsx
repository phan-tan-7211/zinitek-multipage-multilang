      ...
      <div className="grid lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-8">
          ...
          <motion.div
            ...
            className="relative bg-card border border-border/50 rounded-2xl overflow-hidden mb-6 aspect-video flex items-center justify-center shadow-xl shadow-black/5 dark:shadow-none"
          >
            <SanityImage
              ...
              {danhSachAnhGallery.map((hinhAnhMuc) => (
                <div
                  ...
                  className={cn(
                    ...
                    hinhAnhHienTai?._id === hinhAnhMuc._id ? "border-[#f97316] ring-2 ring-[#f97316]/20" : "border-border/50 opacity-60 hover:opacity-100"
                  )}
                >
                  <SanityImage
          ...
          <Card className="p-8 bg-card/80 backdrop-blur-md border border-border/50 mt-8 rounded-2xl shadow-xl shadow-black/5 dark:shadow-none">
            ...
            <p className="text-muted-foreground mb-8 leading-relaxed text-lg font-medium opacity-90">
        ...
        <div className="lg:col-span-4 sticky top-28 h-fit space-y-8">
          ...
          <Card className="p-6 bg-card border border-border/50 rounded-2xl shadow-xl shadow-black/5 dark:shadow-none">

            <h3 className="text-2xl font-serif font-bold text-[#f97316] mb-6 border-b border-border/50 pb-4 uppercase tracking-tight">
            ...
            {sanPhamAnToan.specifications.length > 0 ? (
              <div className="space-y-4">
                {sanPhamAnToan.specifications.map((thongSo, chiSo) => (
                  <div key={chiSo} className="flex justify-between items-start gap-4 text-sm border-b border-border/30 pb-3 last:border-b-0 last:pb-0">
                    <span className="text-muted-foreground font-bold uppercase text-[10px] tracking-widest mt-1">{thongSo.label}</span>
                    <span className="text-foreground font-black text-right">{thongSo.value}</span>
                  </div>
                ))}
              </div>
            ) : (
          ...
          {sanPhamAnToan.attachments.length > 0 && (
            <Card className="p-6 bg-card border border-border/50 rounded-2xl shadow-xl shadow-black/5 dark:shadow-none">
              <h3 className="text-2xl font-serif font-bold text-[#f97316] mb-4 border-b border-border/50 pb-2 uppercase tracking-tight">
              ...
              <div className="space-y-3">
                {sanPhamAnToan.attachments.map((tepTin) => (
                  <a
                    ...
                    className="flex items-center justify-between p-4 bg-secondary rounded-xl border border-border/50 hover:border-[#f97316]/50 hover:bg-muted transition-all duration-300 group"
                  >
